...
def load_transpose_CER(
    file_x, file_case=None, mask="id_pdl", scale_data=False, scaler=StandardScaler()
):
    ...
    if scale_data:
        df_data_x = pd.DataFrame(
            scaler.fit_transform(df_data_x.T).T,
            columns=df_data_x.columns,
            index=df_data_x.index,
        )
...
def create_X_out_of_df(df: pd.DataFrame) -> np.ndarray:
    ...
    if df.isna().any().any():
        print("At utils.create_X_out_of_df: check the dataframe, there are NaN values!")
        print("NaN values for the dataframe are temporarily filled with 0!")
        df = df.fillna(0)
